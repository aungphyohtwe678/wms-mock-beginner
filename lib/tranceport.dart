...
class PalletLabelScanScreen extends StatefulWidget {
  final int currentStep;

  const PalletLabelScanScreen({super.key, this.currentStep = 1});

  @override
  State<PalletLabelScanScreen> createState() => _PalletLabelScanScreenState();
}

class _PalletLabelScanScreenState extends State<PalletLabelScanScreen> {
  ...
  void _goToNextScreen() async {
    final result = await Navigator.push(
      context,
      PageRouteBuilder(
        pageBuilder: (context, animation, secondaryAnimation) => LiftScanScreen(currentStep: widget.currentStep),
    ...
    if (result == 'clear') {
      if (widget.currentStep >= 5) {
        ...
        Navigator.pushReplacement(
          context,
          PageRouteBuilder(
            pageBuilder: (context, animation, secondaryAnimation) => PalletLabelScanScreen(currentStep: widget.currentStep + 1),
            transitionDuration: Duration.zero,
            reverseTransitionDuration: Duration.zero,
          ),
        );
